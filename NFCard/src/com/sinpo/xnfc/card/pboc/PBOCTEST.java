...
final class PBOCTEST extends PbocCard {
	...
	private PBOCTEST(Iso7816.Tag tag, Resources res) {
		super(tag);
		name = res.getString(R.string.name_pboctest);
	}

	@SuppressWarnings("unchecked")
	final static PBOCTEST load(Iso7816.Tag tag, Resources res) {
		...
		if (tag.selectByName(ISD_AID).isOkey()) {
			...
			if (tag.selectByName(DFN_SRV).isOkey()) {
				...
				// build result string
				/*--------------------------------------------------------------*/
				final PBOCTEST ret = new PBOCTEST(tag, res);
